...
class ForecastViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let cairo = Location(name: "Cairo, Egypt", latitude: 30.0626, longitude: 31.2497)
        locations = [cairo, sanJose, manila, italy]
        changeLocation(withLocationIndex: selectedLocationIndex)
    }

    private func changeLocation(withLocationIndex index: Int) {
        guard index < locations.count else { return }
        let location = locations[index]
        locationLabel.text = location.name
        WeatherForecastService.fetchForecast(
